    ...
    <div class="col-12 col-md-8 col-lg-6">
      <form action="/admin/categories/{{ $category->id }}" method="post" class="mt-3">
        ...
        <div class="form-group">
          ...
          <input type="text" name="name" id="name" class="form-control rounded-pill border-dark" placeholder="حواسيب تصميم" value="{{ $category->name }}">
          @error('name')
        ...
        <div class="form-group">
          ...
          <textarea id="description" name="description" class="form-control rounded border-dark" cols="30" rows="10" placeholder="وصف الحواسيب ضمن هذه الفئة">{{ $category->description }}</textarea>
          @error('description')
                <small class="text-danger">{{ $message }}</small>
          @enderror
        </div>
